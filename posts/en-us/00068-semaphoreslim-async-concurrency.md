...
👉 Unlike `lock` (always 1 at a time), `SemaphoreSlim` can allow **N** simultaneous operations. This is exactly what you'd use to limit how many concurrent HTTP calls your code makes to an external API, avoiding overloading it or blowing past rate limits (remember post 56?)
