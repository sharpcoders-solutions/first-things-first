...
- Operator overloading and how to teach the compiler new behaviors (post 56)  
...
- Confusing `GetAwaiter()` (which enables `await`) with `GetEnumerator()` (which enables `foreach`, from post 55) — similar patterns, but for completely different purposes  
