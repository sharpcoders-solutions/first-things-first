...
Remember `record`s (post 27)? Property patterns pair perfectly with them — records are often used exactly to model the data these patterns inspect
...
👉 Remember tuples (post 48)? Combined with `switch`, they let you test multiple related values in a single pattern — including `when` clauses for conditions that don't fit a simple structural pattern
