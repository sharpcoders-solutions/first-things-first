...
- Records and immutability (post 27), LINQ (post 19), delegates and lambdas (post 25)  
...
```csharp
// We've already used this since post 27
...
👉 Pure functions are easier to test (remember post 30?) — no mocks, no setup, just input and output
...
👉 Instead of throwing exceptions for expected control flow (remember post 23, about when to use exceptions?), the `Result` pattern makes the error path explicit in the return type — the caller is forced to handle both cases
