...
👉 `IClassFixture<TestFactory>` shares the same application instance across all tests in the class, avoiding the cost of spinning up the application for every test — each `[Fact]` remains isolated (post 33), but the infrastructure is reused
