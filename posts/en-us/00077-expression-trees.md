...
- LINQ (post 19) and Entity Framework Core (post 35)  
...
👉 Remember post 35? EF Core's `DbSet<T>` implements `IQueryable<T>`, which uses `Expression<Func<>>` instead of plain `Func<>` — that's why EF can "read" your LINQ and translate it to SQL, instead of downloading all the data and filtering in memory
