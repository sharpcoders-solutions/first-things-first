...
- RabbitMQ (post 44) for messaging between different services  
...
👉 `Channel<T>` is safe for multiple simultaneous producers and consumers, and integrates natively with `async`/`await` (post 29) — no manual locks needed
...
- Using Channels when the real problem requires persistence across processes — in that case, RabbitMQ or Outbox (post 61) are the right tools  
