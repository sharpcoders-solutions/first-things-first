...
- Unsafe code and pointers (post 69), including a quick introduction to P/Invoke  
...
👉 `[StructLayout(LayoutKind.Sequential)]` guarantees the struct's fields sit in memory in exactly the same order and layout the native library expects — remember the value types post (post 46)? This is a case where the exact memory layout, usually an internal runtime concern, becomes your explicit responsibility
...
Just like the other source-generator-based features you've already seen, `[LibraryImport]` generates marshaling code at **compile** time, instead of using reflection at runtime — faster, and compatible with Native AOT (post 72), where traditional `[DllImport]` can have limitations
