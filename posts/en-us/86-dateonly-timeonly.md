...
- Serverless on both AWS and Azure, with the same fundamentals applied on different clouds  
...
👉 Remember EF Core (post 32)? Since EF Core 6, `DateOnly` and `TimeOnly` are natively mapped to SQL Server's `date` and `time` types — no need for custom converters, and no waste storing a date or time part that will never be used
