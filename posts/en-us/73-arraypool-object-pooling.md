...
- Span and Memory for accessing memory without extra allocations (post 49)  
...
👉 ASP.NET Core itself uses Object Pooling internally for `StringBuilder` (remember post 49?) and HTTP response buffers — each request doesn't allocate these objects from scratch, they're borrowed and returned to the pool automatically
