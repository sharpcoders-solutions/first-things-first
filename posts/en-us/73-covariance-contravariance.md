...
- Generics (post 24) and inheritance/polymorphism (post 21)  
...
👉 Even though `Dog` is an `Animal` (remember inheritance from post 21?), `List<Dog>` is **not** automatically a `List<Animal>` — because lists are mutable, and allowing this would break type safety
