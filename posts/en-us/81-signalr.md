...
👉 A `Hub` keeps connections open (via WebSockets, with automatic fallback) — unlike a REST controller (post 31), which responds and closes the connection, the Hub keeps the channel open for the server to speak whenever it wants
...
- Sending sensitive data without authentication on the Hub — remember JWT (post 34), Hubs need authorization too  
