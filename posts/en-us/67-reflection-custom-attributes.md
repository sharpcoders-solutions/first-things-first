...
👉 A custom attribute is nothing more than a class that inherits from `Attribute` — the same mechanism behind `[Fact]` (post 30) and `[Authorize]` (post 34)
...
- Using Reflection to bypass encapsulation (accessing `private` fields from outside), breaking the OOP principles from post 20  
