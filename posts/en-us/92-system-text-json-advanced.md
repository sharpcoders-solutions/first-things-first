...
- `GeneratedRegex` and how source generators eliminate runtime cost  
...
👉 ASP.NET Core already uses `System.Text.Json` under the hood in every `[HttpPost]`/`[HttpGet]` you've written — the `record` (post 27) maps naturally to JSON, field by field
...
👉 **`JsonSerializerContext` = a source generator (remember post 68 and `GeneratedRegex`?) that generates, at compile time, all the serialization code for the declared types**

Without this feature, `System.Text.Json` uses **reflection** (post 67) to discover properties at runtime — it works, but has a performance cost and, more importantly, **doesn't work with Native AOT** (post 69), because reflecting over types that no longer exist as metadata simply fails
