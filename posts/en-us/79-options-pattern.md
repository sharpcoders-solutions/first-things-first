...
👉 `IOptions<T>` is resolved once, at startup. `IOptionsSnapshot<T>` reloads the value on every request — combined with `reloadOnChange: true` in `appsettings.json`, you change configuration without restarting the application, similar in spirit to the feature flags from post 54
