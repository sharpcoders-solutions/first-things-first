...
👉 Diferente de `lock` (sempre 1 por vez), `SemaphoreSlim` pode permitir **N** operações simultâneas. Isso é exatamente o que você usa para limitar quantas chamadas HTTP simultâneas seu código faz para uma API externa, evitando sobrecarregá-la ou estourar limites de rate limiting (lembra do post 53?)
