...
- LINQ (post 19) e Entity Framework Core (post 35)  
...
👉 Lembra do post 35? O `DbSet<T>` do EF Core implementa `IQueryable<T>`, que usa `Expression<Func<>>` em vez de `Func<>` puro — por isso o EF consegue "ler" seu LINQ e traduzir para SQL, em vez de baixar todos os dados e filtrar em memória
