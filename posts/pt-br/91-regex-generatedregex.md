...
- `required` e `init` para garantir objetos corretamente inicializados  
- Source generators (post 68), gerando código em tempo de compilação  
...
👉 **`[GeneratedRegex]` = um atributo que instrui um source generator (lembra do post 68?) a gerar, em tempo de **compilação**, todo o código C# necessário para aquele padrão — sem interpretar nada em runtime**
...
👉 Com regex otimizado, o próximo passo é olhar para outra biblioteca que você já usa desde os primeiros posts sobre APIs: `System.Text.Json`, e os recursos avançados que vão muito além de `Serialize`/`Deserialize` básicos
