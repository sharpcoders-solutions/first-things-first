...
👉 Essa combinação é o padrão recomendado para DTOs, opções de configuração (lembra do Options Pattern, post 76?) e qualquer classe onde algumas propriedades são essenciais e outras são realmente opcionais — o compilador garante os campos obrigatórios, `init` garante que ninguém muda esses valores depois
...
Aqui você vai aprender a usar expressões regulares em C#, e como o source generator `GeneratedRegex` elimina o custo de compilação em runtime.
