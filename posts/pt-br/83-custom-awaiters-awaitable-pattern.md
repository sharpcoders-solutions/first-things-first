...
- Operator overloading e como ensinar o compilador a entender novos comportamentos (post 59)  
...
- Confundir `GetAwaiter()` (que habilita `await`) com `GetEnumerator()` (que habilita `foreach`, do post 58) — são padrões parecidos, mas para propósitos completamente diferentes  
