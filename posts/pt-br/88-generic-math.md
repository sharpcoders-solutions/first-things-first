...
- A diferença entre `float`, `double` e `decimal`, e quando usar cada um  
- Static abstract interface members (post 58), incluindo o exemplo de `INumber<T>`  
...
👉 `T.Zero` e `T.CreateChecked` também vêm de `INumber<T>` — o valor zero "genérico" e uma forma segura de converter outro tipo numérico (como `int quantidade`) para o tipo `T` sendo usado, sem boxing (lembra do post 44?) e sem casts manuais arriscados
