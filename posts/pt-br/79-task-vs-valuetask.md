...
- Boxing, unboxing e o custo de alocações desnecessárias (post 44)  
...
Lembra da diferença entre value types e reference types (post 43)? Esse é exatamente o motivo pelo qual `ValueTask<T>` existe: evitar a alocação de heap de uma `Task<T>` no caminho onde o valor já está disponível
