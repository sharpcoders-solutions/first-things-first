...
👉 `IOptions<T>` é resolvido uma vez, na inicialização. `IOptionsSnapshot<T>` recarrega o valor a cada requisição — combinado com `reloadOnChange: true` no `appsettings.json`, você muda configuração sem reiniciar a aplicação, parecido com o espírito das feature flags do post 54
...
Aqui você vai aprender a cancelar operações assíncronas de forma cooperativa e correta, além do básico que você já usa desde o post sobre async/await.
