...
- Generics (post 27) e herança/polimorfismo (post 24)  
...
👉 Mesmo `Cachorro` sendo um `Animal` (lembra da herança do post 24?), `List<Cachorro>` **não** é automaticamente um `List<Animal>` — porque listas são mutáveis, e permitir isso quebraria a segurança de tipos
