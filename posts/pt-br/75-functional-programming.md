...
- Records e imutabilidade (post 30), LINQ (post 19), delegates e lambdas (post 28)  
...
```csharp
// Já usamos isso desde o post 30
...
👉 Funções puras são mais fáceis de testar (lembra do post 33?) — sem mocks, sem setup, só entrada e saída
...
👉 Em vez de lançar exceções para fluxo de controle esperado (lembra do post 26, sobre quando usar exceções?), o padrão `Result` torna o caminho de erro explícito no tipo de retorno — o chamador é forçado a lidar com ambos os casos
