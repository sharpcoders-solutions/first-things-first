...
- RabbitMQ (post 44) para mensageria entre serviços diferentes  
...
👉 `Channel<T>` é seguro para múltiplos produtores e consumidores simultâneos, e integra nativamente com `async`/`await` (post 29) — sem precisar de locks manuais
...
- Usar Channels quando o problema real exige persistência entre processos — nesse caso, RabbitMQ ou Outbox (post 61) são as ferramentas certas  
