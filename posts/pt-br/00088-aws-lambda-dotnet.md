...
Os princípios são os mesmos do post 87 — sem gerenciar servidor, escala automática, paga por execução. O que muda é a sintaxe e o ecossistema ao redor
...
👉 Lembra do HTTP Trigger do Azure Functions (post 87)? Aqui, o API Gateway da AWS faz esse papel — recebe a requisição HTTP e invoca a Lambda, parecido em espírito, diferente em implementação
...
👉 Lembra do post 72? A AWS oferece o runtime `provided.al2023` justamente para rodar executáveis Native AOT — cold start em C# tradicional podia ser um ponto fraco frente a linguagens como Go ou Node.js, e o Native AOT fecha essa lacuna quase completamente
...
👉 Uma vez que você entende o modelo mental de serverless (post 87), migrar entre Azure e AWS é principalmente aprender uma nova sintaxe, não um novo paradigma
...
- Native AOT (post 72) é especialmente valioso na AWS, com suporte via runtime `provided.al2023`  
...
Aqui você vai entender como threads realmente enxergam a memória compartilhada, e por que reordenação de instruções pode quebrar código multithread que parece correto.
