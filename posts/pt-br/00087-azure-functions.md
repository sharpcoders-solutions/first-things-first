...
- Deploy contínuo de aplicações .NET completas, sempre rodando (containers, VMs)  
...
👉 Parecido com um endpoint REST (post 34), mas sem toda a infraestrutura do ASP.NET Core rodando continuamente — só ativa quando alguém chama
...
👉 Lembra do Hangfire (post 55)? Isso é conceitualmente parecido, mas sem precisar manter um processo rodando o tempo todo — a nuvem "acorda" a função só na hora certa
...
👉 Similar ao consumidor do RabbitMQ (post 44), mas sem você gerenciar a infraestrutura de consumo — a função escala automaticamente conforme a fila cresce

---

# ⚡ Onde o Native AOT (post 72) se conecta
...
👉 Para cargas de trabalho esporádicas ou imprevisíveis, isso muda completamente a equação de custo comparado às abordagens tradicionais de deploy contínuo, sempre ativas
