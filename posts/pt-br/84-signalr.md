...
- Todo o modelo request/response desde o post 34 — o cliente sempre pergunta, o servidor sempre responde  
...
👉 Um `Hub` mantém conexões abertas (via WebSockets, com fallback automático) — diferente de um controller REST (post 34), que responde e encerra a conexão, o Hub mantém o canal aberto para o servidor falar quando quiser
...
## 🔹 REST assíncrono com `Task`/`ValueTask` (posts 81–83)
...
- Enviar dados sensíveis sem autenticação no Hub — lembre-se do JWT (post 37), Hubs também precisam de autorização  
