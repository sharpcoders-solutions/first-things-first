...
```
Navegador ←── SignalR (post 84) ──→ Servidor executa o C#
```
...
👉 `@inject` usa o mesmo container de DI de sempre (post sobre ASP.NET Core) — e `OnInitializedAsync` é o ciclo de vida do componente, parecido com o `ExecuteAsync` do `BackgroundService` (post 55)
