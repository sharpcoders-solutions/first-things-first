...
- Span e Memory para acessar memória sem alocações extras (post 49)  
...
👉 O próprio ASP.NET Core usa Object Pooling internamente para `StringBuilder` (lembra do post 49?) e buffers de resposta HTTP — cada requisição não aloca esses objetos do zero, eles são emprestados e devolvidos ao pool automaticamente
